...
# Search for .bat files in profile folders (minimum, soc-seti, maksimum)
$profileDirs = @("minimum", "soc-seti", "maksimum")
$batFiles = @()
foreach ($profile in $profileDirs) {
    $profilePath = Join-Path $targetDir $profile
    if (Test-Path $profilePath) {
        $batFiles += Get-ChildItem -Path $profilePath -Filter "*.bat" | Where-Object { $_.Name -notlike "service*" }
    }
}
$batFiles = @($batFiles | Sort-Object { (Split-Path $_.FullName -Parent) | Split-Path -Leaf }, { $_.Name })
...
# Select test mode: all configs, profile, or custom subset
function Read-ModeSelection {
    while ($true) {
        ...
        Write-Host "  [3] minimum    - Discord + YouTube" -ForegroundColor Gray
        Write-Host "  [4] soc-seti   - Discord + YouTube + Social networks" -ForegroundColor Gray
        Write-Host "  [5] maksimum   - Full list (all services)" -ForegroundColor Gray
...
if ($mode -eq 'select') {
    ...
    $batFiles = @($selected)
} elseif ($mode -match '^(minimum|soc-seti|maksimum)$') {
    $profileFilter = $mode
    $batFiles = @($batFiles | Where-Object { $folder = (Split-Path $_.FullName -Parent) | Split-Path -Leaf; $folder -eq $profileFilter })
    if ($batFiles.Count -eq 0) {
        Write-Host "[ERROR] No .bat files found in profile '$mode'" -ForegroundColor Red
        Write-Host "Press any key to exit..." -ForegroundColor Yellow
        [void][System.Console]::ReadKey($true)
        exit 1
    }
    Write-Host "[INFO] Running tests for profile: $mode ($($batFiles.Count) configs)" -ForegroundColor Gray
}
...
try {
    ...
    $profileName = (Split-Path $file.FullName -Parent) | Split-Path -Leaf
    $displayName = if ($profileName -match "^(minimum|soc-seti|maksimum)$") { "$profileName\$($file.Name)" } else { $file.Name }
    Write-Host "  [$configNum/$($batFiles.Count)] $displayName" -ForegroundColor Yellow
    Write-Host "------------------------------------------------------------" -ForegroundColor DarkCyan
    ...
    $batWorkingDir = Split-Path $file.FullName -Parent
    $proc = Start-Process -FilePath "cmd.exe" -ArgumentList "/c `"$($file.FullName)`"" -WorkingDirectory $batWorkingDir -PassThru -WindowStyle Minimized
