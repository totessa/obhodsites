...
echo      3. Check Status
echo      12. Switch Profile
echo.
echo   :: SETTINGS
echo      4. Game Filter         [!GameFilterStatus!]
if defined INSTALLED_PROFILE_FOLDER (
    echo      5. IPSet Filter        [!IPsetStatus!] [profile: !INSTALLED_PROFILE_FOLDER!]
) else (
...
if "%menu_choice%"=="12" goto service_switch_profile
...
:: Profile selection
echo Select profile:
echo   1. minimum    - Discord + YouTube
echo   2. soc-seti   - Discord + YouTube + Social networks
echo   3. maksimum   - Full list (all services)
echo.
set "profile_choice="
set /p "profile_choice=Input profile (1-3): "
if "%profile_choice%"=="1" set "PROFILE_FOLDER=minimum"
if "%profile_choice%"=="2" set "PROFILE_FOLDER=soc-seti"
if "%profile_choice%"=="3" set "PROFILE_FOLDER=maksimum"
if not defined PROFILE_FOLDER (
    echo Invalid choice, exiting...
    pause
    goto menu
)
if not exist "%~dp0%PROFILE_FOLDER%\" (
    echo Profile folder not found: %PROFILE_FOLDER%
    pause
    goto menu
)

set "LISTS_PATH=%~dp0%PROFILE_FOLDER%\lists\"

:: Searching for .bat files in profile folder, except files that start with "service"
echo.
echo Pick strategy:
set "count=0"
for /f "delims=" %%F in ('powershell -NoProfile -Command "Get-ChildItem -LiteralPath '%~dp0%PROFILE_FOLDER%' -Filter '*.bat' | Where-Object { $_.Name -notlike 'service*' } | Sort-Object { [Regex]::Replace($_.Name, '(\d+)', { $args[0].Value.PadLeft(8, '0') }) } | ForEach-Object { $_.Name }"') do (
    set /a count+=1
    echo !count!. %%F
    set "file!count!=%%F"
)
...
for /f "tokens=*" %%a in ('type "%~dp0%PROFILE_FOLDER%\!selectedFile!"') do (
    set "line=%%a"
...
reg add "HKLM\System\CurrentControlSet\Services\zapret" /v zapret-discord-youtube /t REG_SZ /d "!PROFILE_FOLDER!\!filename!" /f
...
if defined INSTALLED_PROFILE_FOLDER (
    if exist "%~dp0%INSTALLED_PROFILE_FOLDER%\lists\ipset-all.txt" (
        set "listFile=%~dp0%INSTALLED_PROFILE_FOLDER%\lists\ipset-all.txt"
    ) else (
...
if defined INSTALLED_PROFILE_FOLDER (
    if exist "%~dp0%INSTALLED_PROFILE_FOLDER%\lists\ipset-all.txt" (
        set "listFile=%~dp0%INSTALLED_PROFILE_FOLDER%\lists\ipset-all.txt"
        echo Using profile: %INSTALLED_PROFILE_FOLDER%
    ) else (
        set "listFile=%~dp0lists\ipset-all.txt"
        echo Profile folder not found, using root lists
    )
) else (
    set "listFile=%~dp0lists\ipset-all.txt"
    echo No profile installed, using root lists
)
echo.
...
call :get_installed_profile
if defined INSTALLED_PROFILE_FOLDER (
    if exist "%~dp0%INSTALLED_PROFILE_FOLDER%\lists\" (
        set "listFile=%~dp0%INSTALLED_PROFILE_FOLDER%\lists\ipset-all.txt"
        echo Updating ipset for profile: %INSTALLED_PROFILE_FOLDER%
    ) else (
        set "listFile=%~dp0lists\ipset-all.txt"
        echo Profile folder not found, updating root lists
    )
) else (
    set "listFile=%~dp0lists\ipset-all.txt"
    echo No profile installed. Updating root lists.
)
...
set "INSTALLED_PROFILE_FOLDER="
set "regval="
sc query "zapret" >nul 2>&1
if errorlevel 1 exit /b
for /f "skip=1 tokens=3*" %%A in ('reg query "HKLM\System\CurrentControlSet\Services\zapret" /v zapret-discord-youtube 2^>nul') do set "regval=%%B"
if not defined regval exit /b
for /f "tokens=1 delims=\" %%P in ("!regval!") do (
    if "%%P"=="minimum" set "INSTALLED_PROFILE_FOLDER=minimum"
    if "%%P"=="soc-seti" set "INSTALLED_PROFILE_FOLDER=soc-seti"
    if "%%P"=="maksimum" set "INSTALLED_PROFILE_FOLDER=maksimum"
)
exit /b
...
sc query "zapret" >nul 2>&1
if errorlevel 1 (
    echo No service installed. Running Install Service...
    echo.
...
echo.
echo Installing with new profile...
echo.
goto service_install
